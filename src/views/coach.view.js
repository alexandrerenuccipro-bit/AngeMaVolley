...
// ── LAYOUT PARTAGÉ ──────────────────────────────────────────────
function renderCoachLayout({ title, user, content }) {
  return `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
...
// ── PAGE ÉQUIPE ─────────────────────────────────────────────────
function renderCoachEquipe({ user, profil, equipe }) {
  const profilSection = profil ? `
    <div class="grid-cards" style="margin-bottom:2rem;">
      <article class="info-card">
        <h3>Mon profil</h3>
        <ul style="list-style:none; padding:0;">
          <li><strong>Diplôme :</strong> ${escapeHtml(profil.diplome || '—')}</li>
          <li><strong>Spécialité :</strong> ${escapeHtml(profil.specialite || '—')}</li>
          <li><strong>Années d'expérience :</strong> ${profil.annees_experience ? profil.annees_experience + ' ans' : '—'}</li>
      ...
      <article class="info-card">
        ...
        <ul style="list-style:none; padding:0;">
          ...
          <li><strong>Catégorie :</strong> ${escapeHtml(equipe.categorie || '—')}</li>
          <li><strong>Joueurs :</strong> ${equipe.nb_joueurs}/${equipe.nb_joueurs_max}</li>
          <li><strong>Créée le :</strong> ${formatDate(equipe.date_creation)}</li>
        </ul>
      </article>

      <article class="info-card">
        <h3>Coordonnées club</h3>
        <ul style="list-style:none; padding:0;">
          <li><strong>Nom :</strong> ${escapeHtml(equipe.nom_club || '—')}</li>
  ...
  const content = `
    <section class="hero-card" style="margin-bottom:1.5rem;">
      <p class="tag">Espace coach</p>
      <h2>Mon équipe</h2>
      <p>Retrouve ici toutes les informations sur ton équipe.</p>
    </section>
    ${profilSection}
    ${couleurPreview}
  `;

  return renderCoachLayout({ title: 'Mon équipe', user, content });
}

// ── PAGE JOUEURS ────────────────────────────────────────────────
function renderCoachJoueurs({ user, equipe, joueurs }) {
  const rows = joueurs.length ? joueurs.map(j => `
    <tr>
      <td>
        ${escapeHtml(j.prenom)} ${escapeHtml(j.nom)}<br>
        <small style="color:var(--muted-text)">${escapeHtml(j.email)}</small>
      </td>
      <td>${escapeHtml(j.taille_cm) ? escapeHtml(String(j.taille_cm)) + ' cm' : '—'}</td>
      <td>${escapeHtml(j.poids_kg) ? escapeHtml(String(j.poids_kg)) + ' kg' : '—'}</td>
      <td><span class="status-badge status-${escapeHtml(j.statut)}">${escapeHtml(j.statut)}</span></td>
      <td>${escapeHtml(j.nom_club || '—')}</td>
    </tr>
  `).join('') : `<tr><td colspan="6" style="text-align:center; color:var(--muted-text); padding:1.5rem;">Aucun joueur.</td></tr>`;

  const content = `
    <section class="hero-card" style="margin-bottom:1.5rem;">
      <p class="tag">Espace coach</p>
      <h2>Mes joueurs</h2>
      <p>Gère and consulte toutes les informations de tes joueurs pour ${escapeHtml(equipe.nom)}.</p>
    </section>

    <section class="equipe-section">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
        <h3 style="font-size:1.3rem; margin:0;">Joueurs (${joueurs.length})</h3>
      </div>

      <div class="table-wrapper">
        <table class="team-table">
          <thead><tr>
            <th>Joueur</th><th>Taille</th><th>Poids</th>
            <th>Statut</th><th>Club</th>
          </tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
  ...
  return renderCoachLayout({ title: 'Mes joueurs', user, content });
}

// ── PAGE ÉVÉNEMENTS ─────────────────────────────────────────────
function renderCoachEvents({ user, equipe, events }) {
  ...
  function renderEventRows(list) {
    if (!list.length) {
      return '<tr><td colspan="5" style="text-align:center; color:var(--muted-text); padding:1.5rem;">Aucun événement.</td></tr>';
    }
    return list.map(e => `
      <tr>
        ...
        <td>${e.score ? escapeHtml(String(e.score)) : '—'}</td>
      </tr>
    `).join('');
  }
  ...
  const content = `
    <section class="hero-card" style="margin-bottom:1.5rem;">
      ...
      <p>Tous les matchs, tournois et entraînements de ${escapeHtml(equipe.nom)}.</p>
    </section>
  ...
  return renderCoachLayout({ title: 'Mes événements', user, content });
}

// ── PAGE LICENCES ────────────────────────────────────────────────
function renderCoachLicences({ user, licences }) {
  ...
  return renderCoachLayout({ title: 'Mes licences', user, content });
}
